...
def slice_overlapped(
    ...
    Args:
       filename (str):  The filename of the image to split.
       window_size(int,int):  x and y of window
       window_overlap(float): the percentage of overlap i.e. 0.5 = 50% overlap
    Kwargs:
       save (bool): Whether or not to save tiles to disk.
       DecompressionBombWarning (bool): Whether to suppress
       Pillow DecompressionBombWarning
    ...
    X_points = start_points(im_w, window_size[0], window_overlap)
    Y_points = start_points(im_h, window_size[1], window_overlap)

    columns = len(X_points)
    rows = len(Y_points)

    # this may not be neccesary but it does validate the size??
    # tile_w, tile_h = int(floor(im_w / columns)), int(floor(im_h / rows))

    tiles = []
    number = 1
    for pos_y in Y_points:  
        for pos_x in X_points:  
            area = (pos_x, pos_y, pos_x + window_size[0], pos_y + window_size[1])
            image = im.crop(area)
            position = ([i for i,x in enumerate(X_points) if x == pos_x][0] + 1, [i for i,x in enumerate(Y_points) if x == pos_y][0] + 1)
            coords = (pos_x, pos_y)
            tile = Tile(image, number, position, coords)
            tiles.append(tile)
            number += 1
    if save:
        save_tiles(
            tiles, prefix=get_basename(filename), directory=directory
        )
    return tuple(tiles)
